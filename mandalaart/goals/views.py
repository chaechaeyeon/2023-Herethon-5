...
def sub_goal_input(request, plan_id):
    ...
    if request.method == 'POST':
        ...
        sub_goal_initial_values = SubGoal.objects.filter(plan=plan).values_list('sub_goal', flat=True)
...
def goal_edit(request, plan_id):
    plan = Plan.objects.get(pk=plan_id)
    sub_goal_ids = plan.sub_goals.values_list('id', flat=True)[:8]  # 최대 8개의 SubGoal ID 가져오기
    
    print(sub_goal_ids)  # Sub Goal ID 출력

    sub_goals = SubGoal.objects.filter(id__in=sub_goal_ids).order_by('id')  # SubGoal을 ID 기반으로 가져오기
    ...
    return render(request, 'goal_edit.html', {'sub_goals': sub_goals, 
...
def three_by_three_table(request, plan_id):
    ...
    return render(request, '3x3_table.html', {'plan': plan, 'sub_goal_rows': sub_goal_rows, 'main_goal': plan.main_goal})
